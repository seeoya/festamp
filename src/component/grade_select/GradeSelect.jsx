...
const GradeSelect = (props) => {
    ...
    useEffect(() => {
        ...
        let starObjInStorage = localStorage.getItem('starDB');
        let curStarDBObj = JSON.parse(starObjInStorage);

        if (star === 1.0) {
            console.log('star 1 CALLED');

            const rootElement = document.getElementById("current_star");
            const element = React.createElement("div", {children : {fullStar}})
    ...
    useEffect(() => {
        ...
        if (starDBInStorage === null) {
            ...
            let starDBJsObj = JSON.parse(starDBInStorage);
            starDBJsObj[reviewNum] = {
                'festivalNum': festivalNum,
                'star': star
            }
        }
